...
namespace Hospitl_Mangement_MVC
{
    ...
    public static class RoleHelper
    {
        ...
        public static void AddAdminRoles(this IServiceProvider serviceProvider)
        {
            ...
            var patientUser = userManger.FindByEmailAsync("patient@gmail.com")?.Result;
            if (patientUser == null)
            {
                patientUser = new BaseEntity()
                {
                    First_Name = "Patient",
                    Last_Name = "User",
                    UserName = "patient",
                ...
                var patientResult = userManger.CreateAsync(patientUser, "Pa@12345").Result;
                if (!patientResult.Succeeded)
                {
                    throw new ArgumentException("patient user not created");
                }
            }

            // أضف دور "Patient" للمستخدم المريض
            var roleResult = userManger.AddToRoleAsync(patientUser, "Patient").Result;
        }
    }
}
